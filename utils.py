...
# ─── Constants ────────────────────────────────────────────────────────────────
DATA_DIR = Path("data")
MASTER_CSV_SH  = DATA_DIR / "shareholding_all.parquet"
MASTER_CSV_FIN = DATA_DIR / "financials_all.parquet"
SNAPSHOT_FILE  = DATA_DIR / "snapshot_all.parquet"
PER_STOCK_SH   = DATA_DIR / "shareholding_screener"
PER_STOCK_FIN  = DATA_DIR / "financials_screener"
PER_STOCK_CA   = DATA_DIR / "corporate_actions_screener"
TV_ALERTS_CSV  = DATA_DIR / "tv_alerts.parquet"
MASTER_CSV_CF  = DATA_DIR / "cashflow_all.parquet"
INSIDER_FILE   = DATA_DIR / "insider_trades.parquet"
PER_STOCK_CF   = DATA_DIR / "cashflow_screener"
BROKERAGE_FILE = DATA_DIR / "brokerage_reports.parquet"
TECHNICAL_FILE = DATA_DIR / "technicals_all.parquet"
CORP_ACTIONS_FILE = DATA_DIR / "corporate_actions_all.parquet"
TV_FILE = DATA_DIR / "tradingview_growth.parquet"
...
# ─── TradingView column normalization ──────────────────────────────────────────
# TradingView's CSV export headers drift slightly between exports (extra spaces,
# commas, wording tweaks). Every page in this app was previously written against
# *literal* hardcoded header strings, so any drift broke pages with a raw
# KeyError. This canonical list + fuzzy matcher fixes that at the source: no
# matter what TradingView calls a column on export day, it gets renamed to the
# exact canonical name below before any page ever sees it.
TV_CANONICAL_COLUMNS = [
    "Symbol",
    "Price",
    "Analyst rating",
    "Sector",
    ...
    "Earnings per share diluted growth %, Quarterly QoQ",
    ...
    "Performance % 1 week",
    "Performance % 1 month",
    "Technical rating 1 day",
    "Technical rating 1 week",
    "Technical rating 1 month",
    "Technical rating 4 hours",
    "Moving averages rating 1 day",
    "Moving averages rating 1 week",
    "Moving averages rating 1 month",
]

def _tv_signature(text):
    """Collapse a column header to a loose comparison key: lowercase,
    alphanumerics only, no spaces/punctuation. Makes 'Performance % 1 month',
    'Performance %, 1 month' and 'performance%1month' all compare equal."""
    import re
    return re.sub(r"[^a-z0-9]", "", str(text).lower())

def normalize_tv_columns(df):
    """Rename whatever columns are actually present in a TradingView export
    to the canonical TV_CANONICAL_COLUMNS names, matched fuzzily. Columns that
    don't match anything canonical are left untouched (not dropped) so nothing
    is silently lost — they just won't be relied upon by name elsewhere."""
    if df.empty:
        return df
    df = df.copy()
    df.columns = [str(c).strip() for c in df.columns]

    canonical_by_sig = {_tv_signature(c): c for c in TV_CANONICAL_COLUMNS}
    rename = {}
    for actual_col in df.columns:
        sig = _tv_signature(actual_col)
        if sig in canonical_by_sig and actual_col != canonical_by_sig[sig]:
            rename[actual_col] = canonical_by_sig[sig]
    if rename:
        df = df.rename(columns=rename)
    return df

def tv_missing_columns(df, required):
    """Return the subset of `required` canonical columns not present in df,
    after normalization. Use this to show a clear warning instead of crashing."""
...
def load_insider_trades():
    if not INSIDER_FILE.exists():
        return pd.DataFrame()
    return pd.read_parquet(INSIDER_FILE)
...
def load_technical_data():
    ...
    try:
        ...
        st.warning(f"Could not load technical data: {e}")
        return pd.DataFrame()
...
def get_file_timestamp(file_path):
    ...
    return datetime.fromtimestamp(p.stat().st_mtime).strftime("%d-%b-%Y %H:%M")
...
def calc_cfo_op(df_cf, df_fin, symbol):
    ...
    op = latest_cf(df_cf, symbol, "Profit from Operations")
...
def _compute_growth_score(sales_yoy, ebitda_yoy, np_yoy, eps_yoy, lat_ebitda, lat_np, lat_eps):
    s = 0.0
...
def calculate_performance_score(df_sh, df_fin, df_cf, df_insider, symbol):
    ...
    cs, cg = _compute_cf_score(cfo, fcf, np_val, cfo_yoy, fcf_yoy, cfo_op)
    ...
    # Re-align non-TV performance calculation to valid composite weights summing to 1.0
    comp     = round(gs * .50 + cs * .35 + os_ * .15, 2)
...
@st.cache_data(ttl=3600)
def build_summary(df_sh, df_fin, df_cf, df_insider, df_snap, df_brokerage, df_tech, df_tv, syms, cats):
    ...
    tech_map = df_tech.set_index("symbol").to_dict("index") if not df_tech.empty else {}
    tv_map = df_tv.set_index("symbol").to_dict("index") if not df_tv.empty else {}
    ...
    if not corp_df.empty:
        latest = corp_df.sort_values("date").groupby("symbol").last().reset_index()
        corp_map = latest.set_index("symbol").to_dict("index")
    ...
    for sym in syms:
        ...
        row["Technical Score"] = tech.get("technical_score")
        row["RSI"] = tech.get("rsi14")
        row["Entry Status"] = tech.get("entry_status")
        row["Close"] = tech.get("close")
        row["EMA50"] = tech.get("ema50")
        row["EMA200"] = tech.get("ema200")
        ...
        cfo_op = calc_cfo_op(df_cf, df_fin, sym)
        fa = cf_latest.get((sym, "Fixed Asset Purchased"))
        cfo_v = cf_latest.get((sym, "CFO"))
        fa_cfo = round(abs(fa) / cfo_v, 2) if fa and cfo_v else None

        row["CFO/OP"] = cfo_op
        row["Cash Quality"] = get_cash_quality_label(cfo_op / 100 if cfo_op is not None else None)
        ...
        fq = sh_qoq.get((sym,"FIIs")) or sh_qoq.get((sym,"FIIs %"))
        dq = sh_qoq.get((sym,"DIIs")) or sh_qoq.get((sym,"DIIs %"))
        pq = sh_qoq.get((sym,"Promoters")) or sh_qoq.get((sym,"Promoters %"))
        os_, sig = _compute_ownership_score(fq, dq, pq, insider_map.get(sym))

        # Dynamically evaluate TradingView score for inclusion in the composite score
        tv_score = 0.0
        if tv:
            try:
                # Scaled raw entry score (up to ~175) down to a normalized 0-10 scale
                tv_score = calculate_entry_score(tv) / 17.5
            except Exception:
                pass

        comp = round(gs*.40 + cs*.30 + os_*0.10 + tv_score*0.20, 2)
        perf, grade = _composite_to_perf_grade(comp)
        ...
        row.update({
            ...
            "FII QoQ":fq,"DIIs QoQ":dq,"Promoters QoQ":pq,"Promoter QoQ":pq,
...
@st.cache_data(ttl=3600)
def build_summary_cached(df_sh, df_fin, df_cf, df_insider, df_snap, df_brokerage, df_tech, df_tv, selected_symbols, selected_cats):
    return build_summary(
        df_sh, df_fin, df_cf, df_insider, df_snap, df_brokerage, df_tech, df_tv, selected_symbols, selected_cats
    )    
...
def analyze_cashflow(row):
    ...
    fa=row.get("Fixed Asset Purchased")
    netc=row.get("Net Cash Flow")
    cop=row.get("CFO/OP")
    cyoy=row.get("CFO YoY %")
    fyoy=row.get("Fixed Asset Purchased YoY %")
    if cop is not None:
...
def latest_shareholding_change(df_sh, symbol, category):
    df_sh = df_sh.copy() # Avoid SettingWithCopyWarning on cached dataframes
    df_sh["category"] = (
        df_sh["category"]
        .astype(str)
        .str.strip()
...
def calculate_trend(df, symbol, metric, periods):
    try:
        if "sort_key" not in df.columns:
            df=df.copy()
            col="period" if "period" in df.columns else "quarter"
            df["sort_key"]=df[col].apply(make_period_key)
    ...
    try:
        ...
        for c in df.columns:
            ...
            if "stock" in lc or "symbol" in lc:
                ...
                rename_map[c] = "broker"
            elif "rating" in lc or "call" in lc or "type" in lc:
                rename_map[c] = "rating"
            elif "target" in lc:
                rename_map[c] = "target_price"
            elif "ltp" in lc or "cmp" in lc or "price" in lc:
                rename_map[c] = "cmp"
            elif "upside" in lc:
        ...
        numeric_cols = ["target_price", "cmp", "upside_pct"]
        ...
        st.error(f"Error loading brokerage data: {e}")
        return pd.DataFrame()
...
def load_corporate_actions():
    if not CORP_ACTIONS_FILE.exists():
        return pd.DataFrame()
    try:
        df = pd.read_parquet(CORP_ACTIONS_FILE)
        df["symbol"] = (
            df["symbol"]
            .astype(str)
            .str.upper()
...
def load_bulk_deals():
    ...
    if not file.exists():
        return pd.DataFrame()
    return prepare_bulk_block(pd.read_parquet(file))
...
def prepare_bulk_block(df):
    ...
    for col in ["Quantity Traded", "Trade Price/Wght. Avg. Price", "Trade_Value"]:
        if col in df.columns:
            df[col] = (
                df[col]
                .astype(str)
    ...
    if (
        ...
        df["Trade_Value"] = df["Quantity Traded"] * df[price_col]
    elif (
        "Trade_Value" in df.columns
        and "Quantity Traded" in df.columns
        and price_col in df.columns
    ):
        computed_value = df["Quantity Traded"] * df[price_col]
        df["Trade_Value"] = df["Trade_Value"].fillna(computed_value)
...
def get_top_bulk_buyers(days=30):
    ...
    buyers = (
        df[df["Buy/Sell"].astype(str).str.upper() == "BUY"]
        .groupby("Client Name")
        .agg(Trade_Value=("Trade_Value", "sum"), Trades=("Symbol", "count"))
        .reset_index()
...
def get_top_bulk_sellers(days=30):
    ...
    df = df[df["Date"] >= cutoff]
    sellers = (
        df[df["Buy/Sell"].astype(str).str.upper() == "SELL"]
        .groupby("Client Name")
        .agg(Trade_Value=("Trade_Value", "sum"), Trades=("Symbol", "count"))
        .reset_index()
        .sort_values("Trade_Value", ascending=False)
...
def get_stock_net_flow(days=30):
    df = pd.concat([load_bulk_deals(), load_block_deals()], ignore_index=True)
    if df.empty:
        return pd.DataFrame()
    cutoff = pd.Timestamp.today() - pd.Timedelta(days=days)
    df = df[df["Date"] >= cutoff]
    ...
    net.columns = ["Buy_Value", "Sell_Value"]
    net["Net_Value"] = net["Buy_Value"] - net["Sell_Value"]
    return net.reset_index().sort_values("Net_Value", ascending=False)
...
def get_column_mapping():
    return {
        ...
        "eps_yoy": "Earnings per share diluted growth %, Quarterly YoY",
        "eps_qoq": "Earnings per share diluted growth %, Quarterly QoQ",
        "rev_yoy": "Revenue growth %, Quarterly YoY",
        "rev_qoq": "Revenue growth %, Quarterly QoQ",
        "rel_vol": "Relative volume 1 day",
        "perf_1w": "Performance % 1 week",
        "perf_1m": "Performance % 1 month",
        "tech_1d": "Technical rating 1 day",
        "tech_1w": "Technical rating 1 week",
        "tech_1m": "Technical rating 1 month",
        "tech_4h": "Technical rating 4 hours",
        "ma_1d": "Moving averages rating 1 day",
        "ma_1w": "Moving averages rating 1 week",
        "ma_1m": "Moving averages rating 1 month",
    }

@st.cache_data
def load_data():
    # Delegates to load_tradingview_data() so column normalization (see
    # normalize_tv_columns) is applied consistently no matter which loader a
    # page calls. Previously this was a separate read of the same parquet
    # file with no normalization, which is why column-name drift broke
    # 3_Market_Scanner.py even after Portfolio_Hub was patched.
    return load_tradingview_data()

def refresh_tradingview_data():
    files = sorted(
    ...
    if not files:
        raise FileNotFoundError("No TradingView CSV found in data folder")
    latest_csv = files[0]
    df = pd.read_csv(latest_csv)
    out_file = DATA_DIR / "tradingview_growth.parquet"
    df.to_parquet(out_file, index=False)
    return latest_csv.name, len(df)

def calculate_entry_score(row):
    """
    Evaluates growth, margin, momentum, and multi-timeframe technical indicator signals.
    Accepts pandas Series, DataFrame rows, or dictionary objects safely.
    """
    score = 0
    ...
    # EPS Growth
    eps_yoy = get_val("Earnings per share diluted growth %, Quarterly YoY")
    if eps_yoy > 20:
        score += 15

    eps_qoq = get_val("Earnings per share diluted growth %, Quarterly QoQ")
    if eps_qoq > 10:
        score += 10

    # Revenue Growth
    rev_yoy = get_val("Revenue growth %, Quarterly YoY")
    if rev_yoy > 15:
        score += 10

    rev_qoq = get_val("Revenue growth %, Quarterly QoQ")
    if rev_qoq > 10:
        score += 5

    # Operating Margin
    op_margin = get_val("Operating margin %, Annual")
    if op_margin > 20:
        score += 10
    elif op_margin > 15:
        score += 5
    ...
    # Momentum
    perf1m = get_val("Performance % 1 month")
    if perf1m > 10:
        score += 10
    elif perf1m > 5:
        score += 5

    perf1w = get_val("Performance % 1 week")
    if perf1w > 3:
        score += 10
    elif perf1w > 1:
        score += 5
    ...
    # Relative Volume
    rel_vol = get_val("Relative volume 1 day")
    if rel_vol > 1.5:
        score += 10
    elif rel_vol > 1.2:
        score += 5
    ...
    # Technical Ratings
    tech1d = get_val("Technical rating 1 day", "")
    if tech1d == "Strong Buy":
        score += 15
    elif tech1d == "Buy":
        score += 10

    tech1w = get_val("Technical rating 1 week", "")
    if tech1w == "Strong Buy":
        score += 10
    elif tech1w == "Buy":
        score += 5

    tech1m = get_val("Technical rating 1 month", "")
    if tech1m == "Strong Buy":
        score += 10
    elif tech1m == "Buy":
        score += 5

    tech4h = get_val("Technical rating 4 hours", "")
    if tech4h == "Strong Buy":
        score += 5
    elif tech4h == "Buy":
        score += 3

    # MA Ratings
    if get_val("Moving averages rating 1 day", "") == "Strong Buy":
        score += 5
    if get_val("Moving averages rating 1 week", "") == "Strong Buy":
        score += 5
    if get_val("Moving averages rating 1 month", "") == "Strong Buy":
        score += 10

    # Analyst Rating
    analyst = get_val("Analyst rating", "")
    if analyst == "Strong Buy":
        score += 10
    elif analyst == "Buy":
        score += 5
    ...
    if (
        tech4h == "Strong Buy"
        and tech1d == "Strong Buy"
        and tech1w == "Strong Buy"
    ):
        score += 25
...
def apply_global_filters(df):
    ...
    min_mcap = st.sidebar.number_input("Min Market Cap", value=1000)
    ...
    if "Sector" in df.columns:
        ...
        st.sidebar.caption("⚠️ 'Sector' column not found in TradingView data — sector filter unavailable.")

    if "Market capitalization" in df.columns:
        df = df[df["Market capitalization"] >= min_mcap]
    else:
        st.sidebar.caption("⚠️ 'Market capitalization' column not found — min market cap filter skipped.")
